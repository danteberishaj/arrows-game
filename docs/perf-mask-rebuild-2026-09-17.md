...
|---|---|---|---|
| JS mask build | **Measured** (EXECUTED, on-device timer) | median 0.040–0.044 ms, p95 0.068–0.097 ms | ≤ 0.3 % |
| Native `setVisibleMask` | **Not measured directly: BLOCKED** (see below) | upper bound from the enclosing Fabric section (INFERRED): median 0.21 ms, max 0.43 ms for a batch that holds only the mask update | ≤ ~2 % |

- **The claim is refuted for the JS half.** It is also refuted, as an upper bound, for the native prop
  apply. Per blocked tap, the two JS builds plus the two mount batches that carry them total at most
  ~1.3 ms (sum of p95s). They run in two frames 340 ms apart. The blocked frame p95 in the same
  sessions was 39.0 and 46.7 ms.
...
- **Still open.** Nobody has measured the direct `ArrowsBoard.setVisibleMask` trace section.
  - The committed Kotlin `Trace` section needs a native (gradle) build, and the host had 3.0–3.3 GB
    free against the 5 GB native-build gate.
  - The captures ran on a JS-only repack over an APK built without that section, so Perfetto
    counted 0 of them.
  - A testable prediction for the unblocked run: 250 arrows, single-instruction mask batches, the
    native section's p95 ≤ 0.43 ms (the enclosing section's max). A larger value would falsify the
    upper-bound reading.
...
| Perfetto `ArrowsBoard.setVisibleMask` (committed in `ArrowsBoardView.kt`) | the `setVisibleMask` body | **none: 0 sections, not in the APK under test** | n/a |
...
APK: JS-only repack (memory note levelplay-emulator-fill-and-js-repack; the P-02 script
`artifacts/P-02/scripts/perf-repack.sh`) over the gradle PERF host
`artifacts/W0-04/perf/apk/fix-gradle-perf.apk`.
...
- **Perfetto:** ran in the background for the whole session (`linux.ftrace` with atrace `gfx`,
  `view`, `input` and `atrace_apps: "com.danteb.arrows"`; ring buffer, write-into-file).
...
- **JS timer counts equal the expected rebuilds in every set** (table below): the instrumented build
  was the one installed, and each blocked tap produced exactly two mask changes.
...
- **Native section count is 0, which is expected, not a pass:** the host APK has no
  `ArrowsBoard.setVisibleMask` section. By the brief's rule, **no native `setVisibleMask` number is
  quoted.** The Fabric section is used only as a bound.
...
JS mismatches: 0. Fabric-batch mismatches: 0.
...
Native `setVisibleMask`, direct: **no data**; n = 0 because the section is not in the APK under test.
Blocked by disk, see "Open".
...
| Set | JS: sum of both builds (n, median, p95, max) | Fabric bound: sum of both mask batches (n, median, p95, max) |
|---|---|---|
| S1 benchmark, 10 runs, 3827 | 10, 0.0972, 0.1067, 0.1067 | 10, 0.5888, 1.0167, 1.0167 |
| S2 benchmark, 10 runs, 3827 | 10, 0.1064, 0.1219, 0.1219 | 10, 0.5652, 1.1983, 1.1983 |
| S2 manual single, 3827 | 20, 0.1184, 0.1460, 0.1487 | 20, 0.4693, 0.6139, 0.7335 |
| S2 manual single, level 11 | 20, 0.0426, 0.0581, 0.1650 | 20, 0.3292, 0.7195, 0.8260 |
...
| Session | Frames | UI frame p50 | **p95** | p99 | max | Janky % | JS per tap p95 | Fabric bound per tap p95 | (JS + bound) / p95, INFERRED |
|---|---|---|---|---|---|---|---|---|---|
| S1 | 65 | 8.87 | **46.71** | 60.89 | 60.89 | 30.8 | 0.107 | 1.017 | ≤ 2.4 % |
| S2 | 67 | 9.75 | **38.95** | 60.52 | 60.52 | 34.3 | 0.122 | 1.198 | ≤ 3.4 % |
...
- **Historical range:** these p95 values sit inside the 35.7–55.1 ms range recorded on 2026-09-08,
  so the sessions are comparable.
...
- **Commit frames that change only the mask:** manual single taps, taps 3–20, S2:
  ...
  Inside those frames the prop apply is ≤ 0.43 ms and the JS build ≤ 0.11 ms. **The commit-frame
  overhead is not explained by the O(n) rebuild.**

## Open

1. **BLOCKED: direct native distribution.** Build a PERF APK with gradle so it includes the
   committed `Trace.beginSection("ArrowsBoard.setVisibleMask")`. That needs ≥ 5 GB free; the host had
   3.0–3.3 GB during this task. Then rerun session S2's commands with Perfetto and fill the native
   row. The section count must equal the JS count: 20 / 40 / 20 / 40 / 20. The prediction above is a
   p95 ≤ 0.43 ms for single-instruction batches at 3827.
   - **Fallback, if app sections do not appear on API 31:** the brief's throwaway `System.nanoTime()`
     logcat patch. It also needs a native build.
2. **Attribution of commit-frame cost** (UI ~3.5 ms, RenderThread ~3.8 ms, plus the GPU wait in
   gfxinfo). It is out of scope here and is the next place to look before optimising this path.
3. **UNVERIFIED-DEVICE:** every number above is emulator (host-GPU translation) or Mac node.
